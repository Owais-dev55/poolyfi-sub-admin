...
const EmployeeManagement = () => {
  ...
  const handleStatusToggle = async (employeeId: string) => {
    try {
      ...
      const newStatus =
        currentEmployee.status === "Active" ? "Inactive" : "Active";
      const isActive = newStatus === "Active";

      // Call the API to update the status
      const response = await updateUserStatus(parseInt(employeeId), {
        isActive,
      });
  ...
  const totalEmployees = metaError
    ? employees.length
    : employeeMeta.totalEmployees;
  const drivers = metaError
    ? employees.filter((emp) => emp.role === "Driver").length
    : employeeMeta.totalDrivers;
  const passengers = metaError
    ? employees.filter((emp) => emp.role === "Passenger").length
    : employeeMeta.totalPassengers;
  const activeNow = metaError
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-5">
              <h2 className="text-lg font-semibold text-gray-900 mb-3 lg:mb-0">
                Employee Overview
              </h2>
            ...
            <div className="px-5 py-2 bg-gray-50 border-t border-gray-200">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="px-2 py-1 text-xs bg-[#FFC11E] text-black rounded font-medium">
                    {currentPage}
                  </span>
